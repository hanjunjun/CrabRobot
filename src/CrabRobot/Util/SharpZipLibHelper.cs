...
namespace CrabRobot.Util
{
    public class SharpZipLibHelper
    {
        ...
        public static int FileInZipCount(string strAimPath)
        {
            ...
            try
            {
                ...
                var iNew = Convert.ToInt32(l_New);
                return iNew;
            }
            catch (Exception ex)
            {
        ...
        /// <param name="password">密码</param>
        /// <returns></returns>
        public static bool DecomparessFile(string sourceFile, Action<int, string> UpdateProg, List<string> jumpPath = null, List<string> jumpExtension = null, string destinationDirectory = null, string password = null)
        {
            bool result = false;
            ...
            try
            {
                //var totalLength = FileInZipCount(sourceFile);
                    ...
                    FileAccess.Read, FileShare.Read)))
                {
                    ...
                    while (zipEntry != null)
                    {
                        ...
                        if (jumpPath == null)
                        {
                            ...
                            else
                            {
                                ...
                                if (!string.IsNullOrEmpty(fileName) && fileName.Trim().Length > 0)
                                {
                                    ...
                                    var any = jumpExtension?.Any(x => x.ToLower() == extension);
                                    if (!any.Value)
                                    {
                        ...
                        {
                            var jump = jumpPath.Any(x => zipEntry.Name.Contains(x));
                            if (!jump)
                            {
                                if (zipEntry.IsDirectory)//如果是文件夹则创建
                                {
